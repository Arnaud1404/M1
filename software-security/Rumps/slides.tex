...
\begin{frame}{Architecture : Chiffrement par flot}
    ...
    Le plaintext $m_t$ est chiffré bit à bit avec le keystream $z_t$ :
    ...
    \note{
        ...
        - Sécurité $\equiv$ qualité du keystream $z_t$.
    }
\end{frame}

% --- SLIDE 1: RAW LFSR ---
\begin{frame}{Le Générateur : LFSR}
    Le keystream est produit par un \textbf{LFSR} (Linear Feedback Shift Register) : un automate linéaire sur $\mathbb{F}_2$.
...
\begin{frame}{La Faiblesse : Berlekamp-Massey}
    ...
    \begin{itemize}
        ...
        \item Il suffit d'observer \textbf{$2\ell$ bits} consécutifs du keystream $z_t$ pour retrouver $S^{(0)}$ et prédire toute la suite.
    \end{itemize}
    ...
    \note{
        ...
        - $2\ell$ bits de keystream $\Rightarrow$ reconstruction complète de $f(X)$ et $S^{(0)}$.
...
\begin{frame}{La Solution : Générateur à Combinaison}
    ...
    \begin{tikzpicture}[thick, scale=0.85, every node/.style={transform shape}]
        \tikzset{
            lfsrbox/.style={draw, rounded corners, minimum height=0.7cm, font=\small, align=center, fill=gray!10},
            gbox/.style={draw, rounded corners, fill=blue!10, minimum width=2.2cm, minimum height=3.8cm, font=\large, align=center}
        }

        % LFSR blocks (stacked, different widths)
        \node[lfsrbox, minimum width=5.0cm] (lfsr1) at (0, 1.4)  {$LFSR_1$ \quad (longueur $\ell_1$)};
        \node[lfsrbox, minimum width=4.0cm] (lfsr2) at (0, 0)     {$LFSR_2$ \quad (longueur $\ell_2$)};
        \node[lfsrbox, minimum width=3.0cm] (lfsr3) at (0, -1.4)  {$LFSR_3$ \quad (longueur $\ell_3$)};

        % g block
        \node[gbox] (gbox) at (5.5, 0) {$g$};
        \node[below=0.01cm of gbox, font=\tiny\itshape, text=blue!70!black] {Combinaison non-linéaire};

        % Arrows LFSR -> g (strictly horizontal)
        \draw[-{Latex[length=2mm]}, thick] (lfsr1.east) -- node[above, font=\scriptsize] {$x_1^{(t)}$} (lfsr1.east -| gbox.west);
        \draw[-{Latex[length=2mm]}, thick] (lfsr2.east) -- node[above, font=\scriptsize] {$x_2^{(t)}$} (gbox.west);
        \draw[-{Latex[length=2mm]}, thick] (lfsr3.east) -- node[above, font=\scriptsize] {$x_3^{(t)}$} (lfsr3.east -| gbox.west);

        % Output z_t
        \draw[-{Latex[length=2mm]}, thick, red!70!black] (gbox.east) -- ++(1.2,0) node[right, font=\large] {$z_t$};
    \end{tikzpicture}
    }
    \end{center}

    \begin{block}{Générateur à Combinaison (Extension du Cours p.9)}
        $$z_t = g(x_1^{(t)}, x_2^{(t)}, \dots, x_k^{(t)}) \qquad \text{où } g : \mathbb{F}_2^{k} \to \mathbb{F}_2$$
        Critère de sécurité : $g$ doit garantir l'\textbf{immunité de corrélation}.
    \end{block}

    \note{
        - Architecture : k LFSR indépendants combinés par g (Modèle de Siegenthaler).
        - Réf Cours : Extension du concept de "LFSR filtré" (p.9) à une architecture multi-registres.
        - Argumentaire : Indispensable pour modéliser l'attaque Divide \& Conquer sur Crypto AG.
        - Compromis de Siegenthaler : Ordre d'immunité de corrélation vs Degré algébrique.
        - Sécurité : Repose sur l'impossibilité d'isoler un registre via le keystream.
    }
\end{frame}
...
\begin{frame}{Minerva : La Backdoor Statistique}
    ...
    \vspace{0.3cm}
    \textbf{Conséquence :} Le keystream $z_t$ ``fuite'' de l'information sur la sortie $x_1^{(t)}$ d'un registre individuel.

    \vspace{0.2cm}
    \textit{Ce biais est invisible à l'usage quotidien, mais statistiquement exploitable avec suffisamment de ciphertext.}
...
\begin{frame}{L'Attaque : \textit{Divide \& Conquer}}
    ...
    \begin{columns}
        ...
        \begin{center}
            ...
            \vspace{0.2cm}
            \textcolor{red}{\Large $\mathcal{O}(2^{\ell_1 + \ell_2 + \ell_3})$}\\
            \vspace{0.2cm}
            Espace joint\\
        ...
        \begin{center}
            ...
            \vspace{0.2cm}
            \textcolor{green!60!black}{\Large $\mathcal{O}(2^{\ell_1} + 2^{\ell_2} + 2^{\ell_3})$}\\
            \vspace{0.2cm}
            Registres isolés\\
    ...
    \vspace{0.4cm}
    \textbf{Méthode :} Avec du \textit{plaintext probable} (en-tête diplomatique, salutation standard), l'attaquant compare statistiquement chaque état candidat de $L_1$ au keystream capté. Le vrai état se démarque par corrélation.

    \note{
        - \textbf{Known-Plaintext Attack} : attaque à texte clair connu.
        - Source du plaintext : en-têtes régulières des télex diplomatiques (``To Mr Ambassador...'').
        - $m_t \oplus c_t = z_t$ $\Rightarrow$ keystream récupéré.
...
\begin{frame}[fragile]{Visualisation de l'Attaque en Cascade}
    ...
    \begin{tikzpicture}[auto, thick, node distance=1.5cm and 2cm]
        ...
        \node[draw, rounded corners, fill=blue!10, text width=4.5cm, align=center, below=1.5cm of attack2] (finish) {5. État Initial Reconstruit\\ en cascade (\textbf{Plaintext})};
        ...
        \node[below=0.3cm of attack1, text=red, font=\large] {$\mathcal{O}(2^{\ell_1})$};
        \node[below=0.3cm of attack2, fill=white, inner sep=1pt, text=red, font=\large] {$\mathcal{O}(2^{\ell_2})$};
    \end{tikzpicture}
    }
    \end{center}
    ...
    \begin{enumerate}
        \item Capter le chiffré cible, et y associer du ``Plaintext-Probable'' (en-tête télex, salutation diplomatique).
        \item Simuler $LFSR_1$ pour toutes ses clés partielles ($2^{\ell_1}$ états).
        \item Conserver la clé de $LFSR_1$ qui maximise la corrélation avec le keystream.
        \item Soustraire l'influence de $LFSR_1$ et continuer sur les registres suivants.
    \end{enumerate}

    \note{
        - Attaque en cascade : chaque $LFSR_i$ est cassé séquentiellement.
        - Étape 1 : $c_t \oplus m_t = z_t$ (known-plaintext).
        - Étape 2 : brute-force sur $2^{\ell_1}$ états de $LFSR_1$, corrélation avec $z_t$.
