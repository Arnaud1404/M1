...
\subtitle{Analyse Cryptographique des Machines Crypto AG (Op. RUBICON / MINERVA)}
\author{Arnaud Gomes}
...
\begin{frame}
    ...
    \note{
        ...
        L'histoire que je vais vous raconter aujourd'hui montre que la vulnérabilité technique au sein de ces machines n'était pas une simple erreur de code. Il s'agissait d'une porte dérobée (backdoor) mathématique implantée au niveau matériel, conçue pour affaiblir l'espace des clés de multiples gouvernements.
    }
\end{frame}
...
\begin{frame}{Le Contexte : Les Deux Versions (A et B)}
    ...
    \note{
        ...
        La Version A, sécurisée cryptographiquement, était vendue aux pays alliés de l'OTAN, comme le Royaume-Uni ou les États-Unis.
        La Version B était destinée au reste du monde~: l'Iran, la Libye de Kadhafi, l'Argentine de la junte militaire, l'Inde... La société Crypto AG, désignée par le nom de code MINERVA au sein de la CIA, y implantait une backdoor permettant aux agences de lire les câbles diplomatiques en clair ou presque.
...
\begin{frame}{L'Impact Historique : Exploitation des failles}
    ...
    \note{
        ...
        Lorsque l'Argentine envahit les îles, elle était totalement confiante dans ses communications chiffrées par ses machines Hagelin qu'elle jugeait ultra-modernes. Cette confiance aveugle a entraîné une compromission totale de leurs canaux de communication tactiques et stratégiques. Le Royaume-Uni, en tant que membre de l'alliance Five Eyes, était bénéficiaire direct des interceptions NSA/BND issues des machines compromises vendues par Crypto AG à l'Argentine.
        
        Grâce à la backdoor implantée dans les machines Crypto AG (nom de code MINERVA), la NSA déchiffrait en temps réel les communications navales et diplomatiques argentines et transmettait les renseignements au commandement britannique via les canaux de partage alliés.
    }
\end{frame}
...
\begin{frame}{L'Impact Historique : L'Iran et la Libye}
    ...
    \note{
        ...
        Plus tard, en 1986, lors de la bombe posée dans la discothèque berlinoise ``La Belle'', le président Ronald Reagan ordonne le bombardement de la Libye. Pour justifier cette frappe, l'administration américaine évoque des ``preuves absolues''. Ces preuves irréfutables s'avèrent être les câbles libyens chiffrés (des ``Félicitations pour Berlin'' envoyées de Tripoli) déchiffrés quasi-instantanément par l'infrastructure de la NSA, grâce à la backdoor intentionnelle implantée dans leurs machines Crypto AG.
        
        Une proportion massive du trafic diplomatique du bloc non-aligné a ainsi été systématiquement interceptée et analysée.
    }
\end{frame}
...
\begin{frame}{Architecture : Chiffrement par flot}
    ...
    \note{
        - Cours Chap.~II, Sect.~3~: chiffrement par flot synchrone.
...
\begin{frame}{La Faiblesse : Berlekamp-Massey}
    ...
    \note{
        - Berlekamp-Massey~: Définition~II-8 et Proposition~II-9 (Algorithme de Berlekamp-Massey).
...
\begin{frame}{La Solution : Générateur à Combinaison}
    ...
    \note{
        ...
        - [Transition verbale] «~Il y a un résultat fondamental du cours qui s'applique ici~: le Compromis de Siegenthaler. Pour une fonction booléenne équilibrée à $n$ entrées, on a la borne $m + d \le n - 1$, où $m$ est l'ordre d'immunité de corrélation et $d$ le degré algébrique. En d'autres termes, on ne peut pas maximiser les deux simultanément.~»
        - «~Et c'est exactement ce que la NSA a exploité~: elle a maximisé le degré algébrique $d$ de la fonction de combinaison $g$, de sorte que les tests standard de complexité linéaire (Berlekamp-Massey) restaient incalculables. Cela rassurait les ingénieurs de Crypto AG. Mais en contrepartie, la borne de Siegenthaler impose que $m$ ---~l'immunité de corrélation~--- chute. Et c'est précisément cette faiblesse en corrélation qui ouvre la porte à l'attaque par Divide \& Conquer que nous verrons ensuite.~»
    }
\end{frame}
...
\begin{frame}{La Backdoor Statistique de Crypto AG}
    ...
    \note{
        - La backdoor Crypto AG = violation délibérée de l'immunité de corrélation.
...
\begin{frame}{1992 : La Faille Humaine (L'Affaire Hans Bühler)}
    \begin{columns}
        ...
        \begin{center}
            \begin{alertblock}{La Couverture Parfaite}
                Bühler, comme l'immense majorité des ingénieurs suisses de Crypto AG, ignorait totalement la manipulation des schémas d'immunité des LFSR par la CIA/NSA~!
            \end{alertblock}
        \end{center}
    \end{columns}
    ...
    \note{
        ...
        Ironiquement pour nous analystes d'algorithmes, ce n'est pas un audit cryptographique qui a révélé la faille, mais une grossière erreur humaine~! En mars 1992, Hans Bühler, un des vendeurs phares de Crypto AG qui vendait ces machines au Moyen-Orient, est arrêté par les services iraniens. Les Iraniens s'étaient rendus compte que l'Ouest anticipait de manière trop miraculeuse leurs frappes terroristes et politiques, et ont logiquement pointé du doigt l'intégrité de ces machines suisses.

        Bühler, qui n'était pas au courant de la fraude, reste interrogé pendant neuf mois, jusqu'en janvier 1993. C'est le BND qui paie la rançon d'un million de dollars pour sa libération~; la CIA refuse catégoriquement de contribuer, de peur de ``griller'' la couverture technologique. Ce différend financier entre les deux agences provoque le \textit{Divorce Agreement} de 1993~: la CIA rachète la part allemande pour 17~millions de dollars et prend le contrôle exclusif de Crypto AG.

        Dès sa libération, ce traitement injuste poussera Bühler à alerter la presse germanique. De fil en aiguille, les journalistes d'investigation trouveront la relation cachée entre l'usine de Zoug et le BND à Munich.
    }
\end{frame}
...
\begin{frame}{Références Bibliographiques}
    ...
    \end{itemize}
    \note{
        - Fondements mathématiques~: Chap.~II (Castagnos), Def.~II-4, Prop.~II-5, Def.~II-8 et Prop.~II-9.
