...
\begin{frame}{La Solution : Générateur à Combinaison}
    ...
    \note{
      \begin{itemize}
        ...
        \item \textbf{Siegenthaler :} Borne $m + d \le n - 1$~; impossible de maximiser immunité de corrélation $m$ et degré algébrique $d$ simultanément.
        \item \textbf{Astuce NSA :} $d$ maximisé $\Rightarrow$ Berlekamp-Massey incalculable $\Rightarrow$ ingénieurs Crypto AG rassurés.
        \item \textbf{Conséquence :} $m$ chute $\Rightarrow$ attaque par corrélation (Divide \& Conquer) rendue possible.
      \end{itemize}
    }
\end{frame}
...
\begin{frame}{La Backdoor Statistique de Crypto AG}
    ...
    \note{
      \begin{itemize}
        \item \textbf{Backdoor :} Violation délibérée de l'immunité de corrélation.
        \item \textbf{Façade :} Degré algébrique de $g$ élevé $\Rightarrow$ Berlekamp-Massey incalculable $\Rightarrow$ aspect ``sécurisé''.
        \item \textbf{Faille cachée :} Immunité de corrélation sacrifiée $\Rightarrow$ attaque par corrélation ouverte.
        \item \textbf{Siegenthaler inversé :} Faiblesse dissimulée derrière la complexité apparente.
      \end{itemize}
    }
\end{frame}
