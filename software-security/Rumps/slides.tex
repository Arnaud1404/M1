...
\begin{frame}{Le Générateur : Registres à Décalage (LFSR)}
    ...
    \begin{tikzpicture}[thick, node distance=0.8cm]
        % Styles for uniform circles - Final Tightening
        \tikzset{
            multiplier/.style={draw, circle, inner sep=0pt, minimum size=0.55cm, font=\small},
            xor/.style={draw, circle, inner sep=0pt, minimum size=0.4cm}
        }

        % Register Cells - Slightly shorter
        \node[draw, minimum width=1.4cm, minimum height=0.7cm] (sL-1) at (0,0) {$S_{\ell-1}^{(t)}$};
        \node[draw, minimum width=1.4cm, minimum height=0.7cm, right=0pt of sL-1] (sL-2) {$S_{\ell-2}^{(t)}$};
        \node[draw, minimum width=0.7cm, minimum height=0.7cm, right=0pt of sL-2] (dots1) {$\dots$};
        \node[draw, minimum width=0.7cm, minimum height=0.7cm, right=0pt of dots1] (dots2) {$\dots$};
        \node[draw, minimum width=0.7cm, minimum height=0.7cm, right=0pt of dots2] (dots3) {$\dots$};
        \node[draw, minimum width=1.4cm, minimum height=0.7cm, right=0pt of dots3] (s1) {$S_{1}^{(t)}$};
        \node[draw, minimum width=1.4cm, minimum height=0.7cm, right=0pt of s1] (s0) {$S_{0}^{(t)}$};

        % Multipliers (Circles) - Tighter vertical gap
        \node[multiplier, below=0.4cm of sL-1] (c1) {$c_1$};
        \node[multiplier, below=0.4cm of sL-2] (c2) {$c_2$};
        \node[multiplier, below=0.4cm of s1] (cL-1) {$c_{\ell-1}$};
        \node[multiplier, below=0.4cm of s0] (cL) {$c_\ell$};

        % XOR Gates (Circles with Cross) - Tighter vertical gap
        \node[xor, below=0.4cm of c1] (x1) {$\oplus$};
        ...
        % Connections: Cells to Multipliers
        \draw[-{Latex[length=2mm]}] (sL-1) -- (c1);
        \draw[-{Latex[length=2mm]}] (sL-2) -- (c2);
        \draw[-{Latex[length=2mm]}] (s1) -- (cL-1);
        \draw[-{Latex[length=2mm]}] (s0) -- (cL);

        % Connections: Multipliers to XORs
        \draw[-{Latex[length=2mm]}] (c1) -- (x1);
        \draw[-{Latex[length=2mm]}] (c2) -- (x2);
        \draw[-{Latex[length=2mm]}] (cL-1) -- (xL-1);
        
        % Connection: cL to the chain
        \draw[-] (cL) -- (cL |- x1);
        \draw[-{Latex[length=2mm]}] (cL |- x1) -- (xL-1);

        % Connections: XOR Chain (Right to Left)
        \draw[-{Latex[length=2mm]}] (xL-1) -- ++(-1.4,0); % Pointer into the dots
        \draw[-{Latex[length=2mm]}] (x2) -- (x1);

        % Feedback Loop
        \draw[-] (x1) -- ++(-0.8,0) |- (sL-1.west);

        % Output
        \draw[-{Latex[length=2mm]}] (s0) -- ++(1.0,0) node[right] {$z_t$};
