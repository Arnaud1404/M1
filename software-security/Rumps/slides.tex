...
\begin{frame}{Le Contexte : "Trusted" Hardware}
    \begin{columns}
        ...
        \begin{center}
            ...
            \vspace{0.1cm}
            \tiny \textit{Une machine de la gamme électronique HC-500.}
        \end{center}
    \end{columns}
    ...
    \note{
      \begin{itemize}
        ...
        \item \textbf{Hardware lourd :} Blocs physiques considérés inviolables.
...
\begin{frame}{L'Impact Historique : Exploitation des failles}
    ...
    \begin{itemize}
        ...
        \item \textbf{L'Exploitation :} La junte militaire argentine chiffrait l'intégralité de ses communications navales tactiques avec des machines de la série électronique HC-500.
    ...
    \note{
      \begin{itemize}
        ...
        \item \textbf{Confiance aveugle :} Argentine chiffrait tout via série HC-500 (Version B).
    ...
    \note{
      \begin{itemize}
        ...
        \item \textbf{Stratégie NSA :} Faiblesse statistique dissimulée derrière la complexité apparente de $g$.
                \item \textbf{Widman (alias \og Henry \fg{}) :} Kjell-Ove Widman, mathématicien suédois recruté par la CIA, justifiait académiquement les algorithmes affaiblis auprès des ingénieurs suisses~; le haut degré algébrique apparent de $g$ masquait la vulnérabilité statistique lors des revues internes.
      \end{itemize}
    }
\end{frame}
...
\begin{frame}[fragile]{Visualisation de l'Attaque en Cascade}
    ...
    \begin{tikzpicture}[auto, thick, node distance=1.5cm and 1.5cm]
        ...
        % --- ROW 2 ---
        \node[attack, below=1.4cm of c] (a2) {4. Attaque sur $LFSR_2$\\\textcolor{red}{$\mathcal{O}(2^{\ell_2})$}};
        ...
        % --- ROW 3 ---
        \node[finish, below=1.2cm of dots] (f) {État Reconstruit $\implies$ Clair Identifié};
...
% ==============================================================================
% 6. LA CHUTE DE L'EMPIRE CRYPTO AG
% ==============================================================================
\section{La Chute d'Operation Rubicon}

% --- SLIDE 13 : MENGIA CAFLISCH (années 1980) ---
\begin{frame}{Années 1980 : La Menace Interne (Mengia Caflisch)}
    \vspace{-10pt}
    \begin{columns}
        \column{0.55\textwidth}
        \small
        \textbf{Une ingénieure trop brillante}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Mengia Caflisch}, ingénieure chez Crypto AG, analyse les générateurs HC-500.
            \item Elle identifie des \textbf{faiblesses statistiques suspectes} dans les combineurs non-linéaires.
            \item Elle conçoit un \textbf{algorithme robuste} pour remplacer le générateur compromis.
        \end{itemize}

        \vspace{0.1cm}
        \textbf{La Contre-Attaque de la CIA}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item La CIA panique~: un algorithme robuste neutraliserait la backdoor.
            \item \textbf{Faux audit académique} par \textbf{K-O. Widman} (alias \og Henry \fg{}), mathématicien recruté par la CIA.
            \item Widman invalide le travail de Caflisch et maintient les algorithmes affaiblis.
        \end{itemize}

        \column{0.45\textwidth}
        \begin{center}
            \begin{alertblock}{Le Rôle de Widman}
                \footnotesize
            ...
            \end{alertblock}
            \vspace{0.1cm}
            \begin{block}{La Leçon}
                \footnotesize
                Même une ingénieure compétente à l'intérieur de l'entreprise n'a pas pu briser le cloisonnement.
            \end{block}
        \end{center}
    \end{columns}
    ...
    \note{
      \begin{itemize}
        \item \textbf{Contexte :} Années 1980~; Caflisch travaille sur les générateurs HC-500.
        \item \textbf{Découverte :} Elle repère des biais statistiques dans le combineur $g$.
        \item \textbf{Initiative :} Elle conçoit un algorithme robuste de remplacement.
        \item \textbf{Réponse CIA :} Kjell-Ove Widman (alias \og Henry \fg{}) mène un faux audit pour invalider son travail.
        \item \textbf{Résultat :} La backdoor est préservée~; Caflisch ne sera jamais informée de la vérité.
        \item \textbf{Ironie :} La menace la plus sérieuse pour l'opération ne vient pas d'un adversaire, mais d'une employée consciencieuse.
      \end{itemize}
    }
\end{frame}

% --- SLIDE 14 : HANS BÜHLER (1992) ---
\begin{frame}{1992 : La Faille Humaine (L'Affaire Hans Bühler)}
    ...
    \vspace{0.5cm}
    \begin{center}
        \textit{Ce n'est pas un audit cryptographique qui révèle la faille, mais une erreur humaine et un différend financier entre agences de renseignement.}
    \end{center}
...
% --- SLIDE 15 : DÉCLASSIFICATION 2020 ---
\begin{frame}{2020 : La Déclassification et le Washington Post}
    \begin{columns}
        \column{0.55\textwidth}
        \textbf{L'Enquête \#CRYPTOLEAKS (Février 2020)}
        \begin{itemize}
            \item Enquête conjointe du \textit{Washington Post}, \textit{ZDF} et \textit{SRF}.
            \item Documents CIA déclassifiés~: contrôle de \textbf{+40\%} des flux cryptés mondiaux (1970--1993).
            \item Rapport interne CIA~: \og \textbf{Le coup de maître du renseignement du siècle} \fg{}.
        \end{itemize}

        \vspace{0.2cm}
        \textbf{Épilogue}
        \begin{itemize}
            \item Crypto AG dissoute en \textbf{2018}, scindée en deux entités.
            \item Le scandale relance le débat sur le chiffrement propriétaire vs. open-source.
        \end{itemize}
        ...
        \begin{center}
            \includegraphics[height=3.5cm]{images/hagelin.jpg}\\
            \vspace{0.1cm}
            \small \textit{Série HC-500 réputée mathématiquement inviolable.}
        \end{center}
    \end{columns}
    ...
    \note{
      \begin{itemize}
        ...
        \item \textbf{Liquidation :} Crypto AG dissoute en 2018, scindée en CyOne Security et Crypto International.
...
\begin{frame}{Références Bibliographiques}
    \begin{itemize}
        ...
        \begin{itemize}
            \item G. Miller, \textit{\og The intelligence coup of the century \fg{}}, Washington Post, 2020.
            \item T. Pornin, \textit{\og The Swiss Cheese of Cryptography \fg{}}, SSTIC.
...
\begin{frame}
    ...
    \vspace{1cm}
    \normalsize
    \textit{\og Trust, but Verify. \fg{}}\\
    \vspace{0.2cm}
    \small -- \textbf{Ronald Reagan}, sommets nucléaires (1987)
    
    \vspace{0.5cm}
    \normalsize
    \textit{Ou plutôt : \og Don't trust. Open-Source everything. \fg{}}
