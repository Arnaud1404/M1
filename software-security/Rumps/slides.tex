...
\begin{frame}{1992 : La Faille Humaine (L'Affaire Hans Bühler)}
    \begin{columns}
        ...
        \column{0.4\textwidth}
        \begin{center}
            \begin{alertblock}{La Menace Interne}
                \small
                \textbf{Mengia Caflisch} : Ingénieure brillante de Crypto AG. Trouvant les générateurs suspects, elle conçoit un algorithme réellement robuste. La CIA panique et déploie un faux audit académique (via Kjell-Ove Widman (alias \og Henry \fg{})) pour étouffer son travail et sauver l'opération.
            \end{alertblock}
        \end{center}
    \end{columns}
    ...
    \note{
      \begin{itemize}
        ...
        \item \textbf{Fuite médiatique :} Bühler alerte la presse~; lien Zoug--BND découvert.
                \item \textbf{Mengia Caflisch :} L'ingénieure qui a failli tout faire dérailler. Elle crée un code trop fort, obligeant la CIA à recourir au mathématicien K-O. Widman (alias \og Henry \fg{}) pour neutraliser sa rustine.
      \end{itemize}
    }
\end{frame}
...
\begin{frame}{2020 : La Déclassification et le Washington Post}
    \begin{columns}
        ...
        \begin{center}
            \begin{alertblock}{La Menace Interne}
                \small
                	extbf{Mengia Caflisch} : Ingénieure brillante de Crypto AG. Trouvant les générateurs suspects, elle conçoit un algorithme réellement robuste. La CIA panique et déploie un faux audit académique (via Kjell-Ove Widman) pour étouffer son travail et sauver l'opération.
            \end{alertblock}
        \end{center}
        \vspace{0.3cm}
        \textbf{Le Coup de Maître}
        \begin{itemize}
            \item Un rapport interne de la CIA décrit l'opération Rubicon comme \og Le coup de maître du renseignement du siècle \fg{}.
        \end{itemize}
